...
protocol GuideEventEditorHeaderViewDelegate {
    func header_cancelButtonTapped()
    func header_saveButtonTapped()
    func header_titleTapped()
}

class GuideEventEditorHeaderView: UIView {
    
    public var delegate: GuideEventEditorHeaderViewDelegate?
    ...
    override init(frame: CGRect) {
        ...
        self.cancelButton.addTarget(self, action: #selector(GuideEventEditorHeaderView.closeTapped), for: .touchUpInside)
        
        self.saveButton.setTitle("Save", for: .normal)
        self.saveButton.setTitleColor(UIColor(red:1.00, green:0.40, blue:0.40, alpha:1.00), for: .normal)
        self.saveButton.addTarget(self, action: #selector(GuideEventEditorHeaderView.saveTapped), for: .touchUpInside)
        
        self.titleLabel.textColor = UIColor.black
        self.titleLabel.font = UIFont.systemFont(ofSize: 24, weight: UIFontWeightHeavy)
        self.titleLabel.text = "Create spot"
        self.titleLabel.isUserInteractionEnabled = true
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(GuideEventEditorHeaderView.titleTapped))
    ...
    func titleTapped() {
        self.delegate?.header_titleTapped()
    }
